...
def findBestMask(iterations=100., maxDist = 1000., realProfilePath=False,
  ...
  **kwargs): # includes 'pixelscale' (in case the profile is from ellipse) and galaxy and mask parameters (PAmask, PAgalaxy, bagalaxy and mu_0)
  import Deimos_SKiMS_slit__def__v3 as tst
  #
...
def getReff_PA(): # R_eff along the mask direction
#gal_Reff * gal_ba * mask_PA * gal_PA
  ...
  return np.sqrt((Reff_major*np.cos(DPA_mask_gal))**2.+(Reff_minor*np.sin(DPA_mask_gal))**2.)
    ...
    for ii in self.listSlits:
      for side in ['L', 'R']:
        ...
        if save_slit_length:
          ...
          listStrings.append(nameSlit_str+'\t'+RASlit_str+'\t'+DecSlit_str+'\t'+  equinox_str+'\t'+mag_str+'\t'+passband_str+'\t'+pcode_str+'\t'+  sample_str+'\t'+select_str+'\t'+slitPA_str+'\n')
        counter += 1
        #
